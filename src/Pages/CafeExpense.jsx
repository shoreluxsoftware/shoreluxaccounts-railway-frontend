...
const isAdmin = localStorage.getItem("role") === "ADMIN";
  ...
  // 🔥 FETCH CAFETERIA EXPENSES - NEW API
  const fetchExpenses = useCallback(async () => {
    setLoadingTable(true);
    try {
      const response = await fetch(
        `${API_BASE_URL}/staff-management/list-cafeteria-expenses`,
        {
          headers: {
      ...
      console.error("Fetch cafeteria expenses error:", err);
      showToast(err.message || "Failed to load cafeteria expenses", "error");
      setExpenses([]);
    } finally {
  ...
  // 🔥 DELETE CAFETERIA EXPENSE FUNCTION - NEW
const handleDeleteExpense = async (expense) => {
  ...
  if (!isEditable(expense.date)) {
    showToast("Can only delete cafe expenses up to 2 days old", "error");
    return;
  }

  try {
    const response = await fetch(
      `${API_BASE_URL}/staff-management/delete-cafeteria-expense/${expense.id}`,  // 🔥 Your new API
      {
        method: "DELETE",
    ...
    const result = await handleJsonOrError(response);
    showToast(result.message || "Cafe expense deleted successfully!", "success");
    await fetchExpenses();
  } catch (err) {
    console.error("Delete cafe expense error:", err);
    showToast(err.message || "Failed to delete cafe expense", "error");
  }
};


  // 🔥 CREATE CAFETERIA EXPENSE - NEW API
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/staff-management/create-cafeteria-expense`,
        {
          method: "POST",
          headers: getAuthHeaders(),
          body: formData,
      ...
      showToast("Cafe expense added successfully!", "success");
      setForm({
        date: new Date().toISOString().split("T")[0],
        amount: "",
        description: "",
      ...
      console.error("Add cafeteria expense error:", err);
      showToast(err.message || "Failed to add cafe expense", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  // 🔥 RENDER FIELDS (NEW - HANDLES VOUCHER_NO TOO)
  ...
  // 🔥 UPDATE CAFETERIA EXPENSE - NEW API (UPDATED)
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/staff-management/update-cafeteria-expense/${editExpense.id}`,
        {
          method: "PUT",
          headers: getAuthHeaders(),
          body: formData,
      ...
      showToast("Cafe expense updated successfully", "success");
      setEditModalVisible(false);
      setEditExpense(null);
      await fetchExpenses();
    } catch (err) {
      console.error("Update cafeteria expense error:", err);
    ...
    <div className="flex gap-6 bg-[#F1F2F4] px-6 py-8 min-h-screen">
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-gray-300 rounded-xl p-8 shadow-md w-full max-w-lg">
        <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">
          <Plus size={20} /> Add Cafeteria Expense
        </h2>
        {error && (
        ...
        <div className="flex gap-3 mb-6">
          ...
          <div className="flex-1">
            ...
            <input
              value="Cafeteria"
              disabled
              className="w-full border-b-2 border-dotted border-gray-400 p-2 bg-gray-200 text-sm text-orange-700 font-medium"
            />
          </div>
        </div>
        ...
        {form.bill_file && (
          <div className="flex justify-between items-center bg-green-50 p-3 rounded-md border border-green-200 mb-4">
            ...
            <button type="button" onClick={() => removeFile("bill_file")} disabled={loading}>
              <X className="text-red-600 cursor-pointer hover:text-red-800" size={18} />
            </button>
          </div>
        )}
        ...
        <button
          ...
          disabled={loading}
        >
          {loading ? "Adding..." : "Submit Cafe Expense"}
        </button>
      </form>

      {/* 🔥 RIGHT: CAFE EXPENSES TABLE */}
      <div className="flex-1">
        <div className="flex flex-col sm:flex-row justify-between mb-4 gap-3">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <select
              ...
              disabled
            >
              <option>All Cafe Expenses</option>
            </select>
            <select
              value={sortOption}
        ...
        <div className="bg-white border border-gray-300 rounded-xl p-4 shadow-md overflow-auto max-h-[70vh]">
          <h2 className="text-lg font-semibold mb-4 text-center">
            Cafe Expenses ({filteredExpensesCount})
          </h2>
              ...
              {loadingTable ? (
                <tr>
                  <td colSpan="8" className="text-center py-8 text-blue-500">
                    Loading cafe expenses...
                  </td>
                </tr>
              ) : filteredExpenses.length === 0 ? (
                <tr>
                  <td colSpan="8" className="text-center py-8 text-gray-500">
                    {search ? "No matching cafe expenses" : "No cafe expenses added yet"}
                  </td>
                </tr>
              ) : (
                filteredExpenses.map((exp, index) => (
                  <tr key={exp.id} className="hover:bg-gray-50 border-b">
                    ...
                    <td className="border p-3">
                      <span className="px-2 py-1 bg-orange-100 text-orange-800 rounded-full text-xs font-bold">Cafeteria</span>
                    </td>
          ...
          <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl border border-gray-200">
            ...
            <p className="text-sm text-gray-600 mb-6">
              ...
              <span className="text-xs font-mono bg-gray-100 px-2 py-1 rounded mt-1 inline-block">
                Cafe Expense ID: {editExpense?.id}
              </span>
            </p>
            {error && (
            ...
            <div className="flex justify-end gap-3">
              <button
                type="button"
                onClick={cancelEdit}
                className="px-8 py-3 border-2 border-gray-300 rounded-xl cursor-pointer hover:bg-gray-50 transition-all text-sm font-medium"
                disabled={otpLoading}
              >
                Cancel
              </button>
          ...
          <div className="bg-white rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-200">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-bold text-gray-800">
                Edit Cafe Expense <span className="text-sm text-gray-500 ml-2">(ID: {editExpense.id})</span>
              </h3>
              <button onClick={cancelEdit} className="p-2 hover:bg-gray-100 cursor-pointer rounded-lg">
                <X size={24} />
              </button>
            </div>
            ...
            {!isAdmin && <p className="text-[10px] text-gray-500 mt-1">Staff: Today's date only</p>}
              ...
              <div className="flex-1">
                ...
                <input
                  value="Cafeteria"
                  disabled
                  className="w-full border-b-2 border-dotted border-gray-400 p-2 bg-gray-200 text-sm text-orange-700 font-bold"
                />
              </div>
            </div>
